...
import { taskDetailTemplates } from '../data/task-detail-templates';
...
function buildTaskPerformanceRows(
  ...
  return selectedTaskIds.map((taskId) => {
    ...
    const avgDurationSeconds =
      executions.length > 0 ? average(executions.map((execution) => execution.duration)) : fallbackSeconds;
...
export function buildFinalReportBundle({
  ...
  const performanceDurations = taskPerformance
    .map((row) => toSeconds(row.averageDuration))
  ...
  const sampleFailureRatio =
    representativeSamples.length > 0
      ? representativeSamples.filter((sample) => sample.status === 'failed').length /
        representativeSamples.length
      : 0;

  const categoryRiskRatio =
    categorySummary.length > 0
      ? categorySummary.filter((category) => category.successRate < 75).length /
